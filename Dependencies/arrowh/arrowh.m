...
% 
%   Addendum: Ryan Young -- Fixed a bug in the multi-arrow plot capability.
%   In the loop, the specific arrow to be created depending on list of
%   where was never being specifically referenced depending on loop
%   index. Because of this, the computations designed for scalars were
%   getting vector input.
%   Also fixed an error where it would check if x(1) == x(2) and y(1) ==
%   y(2) when number of points provided was more than two! In which case,
%   the that's not at all the real problem
...
if numel(x) == 2 && numel(y) == 2 && x(1) == x(2) && y(1) == y(2),
...
if sum(isnan(clr)) > 0
    error('Color passed in is wrong.');
end
if nargin <= 3
	ArSize = [100,100];
end
...
for Loop = 1:length(Where),
	...
	if (length(x) == length(y)) && (length(x) > 2),
		...
		x1 = x(1); x2 = (1-Where(Loop)/100)*x(1)+Where(Loop)/100*x(2);
		y1 = y(1); y2 = (1-Where(Loop)/100)*y(1)+Where(Loop)/100*y(2);
	end
